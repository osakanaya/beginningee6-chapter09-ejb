 ...
 * クライアント）から、トランザクション属性がNEVERのEJBメソッドを実行した時の
 * トランザクションの振る舞いを確認するテスト。
 *
 */
@RunWith(Arquillian.class)
public class NEVER_CalleeTest {
	private static final Logger logger = Logger.getLogger(NEVER_CalleeTest.class
			.getName());
	 ...
	 */
	@Test
	public void testNEVER_Callee_REQUIRED_Caller_Both_Persist() throws Exception {
		
        ///// テスト /////
		
		try {
			required_callerEJB.NEVER_CalleePersist_REQUIRED_CallerPersist(book, cd);
			fail("Should throw exception");
		} catch (Exception e) {
			// REQUIRED_CallerEJBからNEVER_CalleeEJBへのメソッド呼び出し時にで
		...
		// Book02エンティティもCD02エンティティも永続化されていないことを確認
	 ...
	 */
	@Test
	public void testNEVER_Callee_SUPPORTS_Caller_Both_Persist() throws Exception {
		
        ///// テスト /////
		
		supports_callerEJB.NEVER_CalleePersist_SUPPORTS_CallerPersist(book, cd);
	...
	/**
	 * ２．NEVERの呼び出し先で自力でトランザクション開始→永続化→ロールバックし、
	 * 　　非チェック例外をスローする例
	 * 　　（呼び出し元はその後永続化を行おうとする）
	 * 
	 * 【結果】
	 ...
	 */
	@Test
	public void testNEVER_Callee_Rollback_SUPPORTS_Caller_Persist() throws Exception {
		
        ///// テスト /////
		
		supports_callerEJB.NEVER_CalleePersistAndRollback_SUPPORTS_CallerPersist(book, cd);
	 ...
	 */
	@Test
	public void testNEVER_Callee_Persist_SUPPORTS_Caller_PersistAndRollback() throws Exception {
		
        ///// テスト /////
		
		supports_callerEJB.NEVER_CalleePersist_SUPPORTS_CallerPersistAndRollback(book, cd);
