 ...
 * クライアント）から、トランザクション属性がSUPPORTSのEJBメソッドを実行した時の
 * トランザクションの振る舞いを確認するテスト。
 *
 */
@RunWith(Arquillian.class)
public class SUPPORTS_CalleeTest {
	private static final Logger logger = Logger.getLogger(SUPPORTS_CalleeTest.class
			.getName());
	...
	public static Archive<?> createDeployment() {
		...
		WebArchive archive = ShrinkWrap
				...
				.addPackage(SUPPORTS_CalleeEJB.class.getPackage())
	 ...
	 */
	@Test
	public void testSUPPORTS_Callee_REQUIRED_Caller_Both_Persist() throws Exception {
		
        ///// テスト /////
		
		required_callerEJB.SUPPORTS_CalleePersist_REQUIRED_CallerPersist(book, cd);
	 ...
	 */
	@Test
	public void testSUPPORTS_Callee_Rollback_REQUIRED_Caller_Persist() throws Exception {
		
        ///// テスト /////
		
		try {
			required_callerEJB.SUPPORTS_CalleePersistAndRollback_REQUIRED_CallerPersist(book, cd);
			fail("Should throw exception");
		} catch (Exception e) {
			// Book02エンティティの永続化時にTrasactionRequiredExceptionをcauseとする
			// EJBExceptionがスローされることを確認
	 ...
	 */
	@Test
	public void testSUPPORTS_Callee_Persist_REQUIRED_Caller_PersistAndRollback() throws Exception {
		
        ///// テスト /////
		
		required_callerEJB.SUPPORTS_CalleePersist_REQUIRED_CallerPersistAndRollback(book, cd);
	 ...
	 */
	@Test
	public void testSUPPORTS_Callee_SUPPORTS_Caller_Both_Persist() throws Exception {
		
        ///// テスト /////
		
		supports_callerEJB.SUPPORTS_CalleePersist_SUPPORTS_CallerPersist(book, cd);
	...
	/**
	 * ２．SUPPORTSの呼び出し先で自力でトランザクション開始→永続化→ロールバックし、
	 * 　　非チェック例外をスローする例
	 * 　　（呼び出し元はその後永続化を行おうとする）
	 * 
	 * 【結果】
	 ...
	 */
	@Test
	public void testSUPPORTS_Callee_Rollback_SUPPORTS_Caller_Persist() throws Exception {
		
        ///// テスト /////
		
		supports_callerEJB.SUPPORTS_CalleePersistAndRollback_SUPPORTS_CallerPersist(book, cd);
	 ...
	 */
	@Test
	public void testSUPPORTS_Callee_Persist_SUPPORTS_Caller_PersistAndRollback() throws Exception {
		
        ///// テスト /////
		
		supports_callerEJB.SUPPORTS_CalleePersist_SUPPORTS_CallerPersistAndRollback(book, cd);
